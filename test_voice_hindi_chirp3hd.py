...
list_available_voices.py / test_voice_compare_chirp3hd.py already in this
folder). Queries Google's real ListVoices API for every hi-IN (Hindi,
India) Chirp3-HD voice on your project, then synthesizes the SAME short
Hindi sample line with each one so you can A/B them side by side — same
approach used to pick Fenrir for English narration originally.
...
Output: voice_samples_hindi/<voice-name>_<gender>.mp3 — one file per real
hi-IN Chirp3-HD voice found on your project. Also prints a plain-text
summary at the end (name/gender/status) so you can see what succeeded
without opening every file.

Run:
    venv\\Scripts\\activate
    python test_voice_hindi_chirp3hd.py
...
# A short, recap-narrator-style Hindi line — representative of the kind of
# sentence Stage 3 would actually write, not just a generic "hello world"
# test phrase, so the sample is a fair preview of real narration quality.
SAMPLE_TEXT_HI = (
    "एक रहस्यमय द्वीप पर, हमारा नायक खुद को पूरी तरह से अकेला पाता है। "
    "लेकिन जल्द ही, उसे एहसास होता है कि वह अकेला नहीं है।"
)

OUT_DIR = Path(__file__).resolve().parent / "voice_samples_hindi"
...
def main():
    ...
    locale = settings.LOCALE_BY_LANGUAGE["hi"]  # "hi-IN"
    ...
    print(f"[voices] found {len(voices)} real hi-IN Chirp3-HD voices\n")
    ...
    for name, gender in voices:
        ...
        try:
            audio = _synthesize_with_retry(client, texttospeech, SAMPLE_TEXT_HI, locale, name)
    ...
    print("Play them and tell me which name you like — same voice name works for "
          "Bengali (bn-IN) too, so once you pick one for Hindi it's worth trying "
          "the same name in Bengali before assuming they need to be different.")
